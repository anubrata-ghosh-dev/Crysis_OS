...
export default function CommandCenter() {
  ...
  useEffect(() => {
    ...
    console.log('📡 Admin Page: Setting up real-time listener for incidents...');
    
    // Subscribe to real-time incident updates from Firestore
    const unsubscribe = incidentService.getIncidentsRealtime((incidents) => {
      console.log('📊 Admin Page: Received incidents from Firestore:', incidents);
      if (incidents && incidents.length > 0) {
        console.log('✅ Admin Page: Updating disasters. Count:', incidents.length);
        setDisasters(incidents as Disaster[]);
      } else {
        // Fall back to mock data if no real incidents
        console.log('⚠️ Admin Page: No real incidents, using mock data');
    ...
    return () => {
      console.log('🛑 Admin Page: Cleaning up real-time listener');
      if (unsubscribe) {
        unsubscribe();
      }
    };
