...
export default function DashboardPage() {
  ...
  // Real-time listener for Firestore incidents
  useEffect(() => {
    console.log('📡 Dashboard: Setting up real-time listener for incidents...');
    
    const unsubscribe = incidentService.getIncidentsRealtime((incidents) => {
      console.log('📊 Dashboard: Received incidents from Firestore:', incidents);
      if (incidents && incidents.length > 0) {
        console.log('✅ Dashboard: Updating with real incidents. Count:', incidents.length);
        setDisasters(incidents as Disaster[]);
      } else {
        console.log('⚠️ Dashboard: No real incidents, using mock data');
  ...
  useEffect(() => {
    ...
    console.log('📈 Dashboard: Updated stats. Total disasters:', disasters.length, 'High severity:', highSeverity);
